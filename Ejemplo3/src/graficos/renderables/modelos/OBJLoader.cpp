...
namespace objl {
	...
	void Mesh::ComputeTangentBasis() {
		for (unsigned int i = 0; i < Indices.size(); i += 3) {
			...
			// Shortcuts for UVs
			Vector2& uv0 = Vertices[index0].TextureCoordinate;
			Vector2& uv1 = Vertices[index1].TextureCoordinate;
			Vector2& uv2 = Vertices[index2].TextureCoordinate;
		...
		for (unsigned int i = 0; i < Vertices.size(); i += 1)
		{
			...
			Vector3& t = Vertices[i].Tangent;
			Vector3& b = Vertices[i].Binormal;

			// Gram-Schmidt orthogonalize
			t = (t - n * math::DotV3(n, t));
			t = t / math::MagnitudeV3(t);

			// Calculate handedness
			if (math::DotV3(math::CrossV3(n, t), b) < 0.0f) {
				t = t * -1.0f;
			}
